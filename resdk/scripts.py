    ...
    for sample_n in annotations.keys():
        if set(annotations[sample_n]['FASTQ_PATH'].split(',')).issubset(set(all_new_read_files_uploaded)):
            ...
            if annotations[sample_n]['PAIRED_END'] == 'Y' and annotations[sample_n]['FASTQ_PATH_PAIR']:
                ...
                input_['src2'] = [f for f in os.path.join(genialis_seq_dir, annotations[sample_n]['FASTQ_PATH_PAIR'].split(','))]
            ...
            if data:
                ...
                resolwe.api.presample(presample['id']).patch({'descriptor': presample['descriptor'], 'presample': False})
                # Setting 'presample': False, moves sample from presample to sample endpoint.
